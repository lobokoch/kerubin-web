...
export class ConciliacaoTransacaoService {
	...
	mountAndGetSearchParams(filter: ConciliacaoTransacaoListFilter): HttpParams {
		...
		// conciliadoComMaisDeUmTitulo
		if (filter.customParams && filter.customParams.size > 0) {
      const customParams = filter.customParams;
			const value = this.mapToJson(customParams);
      console.log('customParams1:' + value);

      // value = '[conciliadoComMaisDeUmTitulo=true]';
      console.log('customParams2:' + value);
