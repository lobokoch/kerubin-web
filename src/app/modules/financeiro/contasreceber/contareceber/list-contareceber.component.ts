...
import { CustomContaReceberListService } from './custom-list-contareceber.service';
...
export class ContaReceberListComponent implements OnInit {
	...
	constructor(
	    ...
	    private customContaReceberListService: CustomContaReceberListService,
	    private financeiroContasReceberTranslationService: FinanceiroContasReceberTranslationService,
	    private confirmation: ConfirmationService,
	    private messageHandler: MessageHandlerService
	) { 
		this.customContaReceberListService.setComponent(this);
	}
	...
	applyRuleGridColumnsStyleClass_SituacaoConta(contaReceber: ContaReceber): String {
		const result = this.customContaReceberListService.applyRuleGridColumnsStyleClass_SituacaoConta(contaReceber);
		if(result) {
		...
		if ((contaReceber.contaPaga === false) && moment(contaReceber.dataVencimento).isBetween(moment({h: 0, m: 0, s: 0, ms: 0}), moment({h: 0, m: 0, s: 0, ms: 0}).add(3, 'day'))) {
			return 'kb-conta-badge kb-conta-vence-proximos-3-dias';
		}
		
		if ((contaReceber.contaPaga === false) && moment(contaReceber.dataVencimento).isBetween(moment({h: 0, m: 0, s: 0, ms: 0}), moment({h: 0, m: 0, s: 0, ms: 0}).endOf('week'))) {
	...
	applyRuleAddColumnSituacaoContaGetValue(contaReceber: ContaReceber): String {
	    ...
	    return this.customContaReceberListService.applyRuleAddColumnSituacaoContaGetValue(contaReceber);
